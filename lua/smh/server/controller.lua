...
local function Load(msgLength, player)
    ...
    if loadFromClient then
        ...
        serializedKeyframes = SMH.Saves.Load(path)
    end

    local entities = SMH.Saves.CreateEntities(serializedKeyframes)
    SMH.KeyframeManager.ImportSave(player, serializedKeyframes, entities)
