...
function PANEL:Init()
	...
	self.FileList:Bind(SMH.Data, "LoadFileName", "ListViewSelect");

	self.EntityList = vgui.Create("DListView", self);
	self.EntityList:AddColumn("Entities");
	self.EntityList:SetMultiSelect(false);
	self.EntityList:Bind(SMH.Data, "LoadFileEntities", "ListView");
	self.EntityList:Bind(SMH.Data, "LoadFileEntity", "ListViewSelect");

	self.Load = vgui.Create("DButton", self);
	self.Load:SetText("Load");
	self.Load:Bind(SMH.Data, "Load", "Button");
